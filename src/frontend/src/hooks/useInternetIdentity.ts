...
export function InternetIdentityProvider({
  ...
  // Keep AuthClient in a ref so it is NEVER destroyed (fixes unclickable login after logout)
  ...
  const login = useCallback(() => {
    ...
    // If already authenticated with a valid delegation, just surface it
    if (
  ...
  // Keep client alive on logout -- only clear identity and status
  const clear = useCallback(() => {
    ...
    void client
      ...
      .then(() => {
        ...
        setStatus("idle");
        setError(undefined);
        // intentionally NOT destroying authClientRef
      })
    ...
    void (async () => {
      try {
        ...
        if (isAuthenticated) {
          ...
          setStatus("success"); // FIX: was always "idle", breaking login on fresh open
        } else {
          setStatus("idle");
        }
      } catch (unknownError) {
